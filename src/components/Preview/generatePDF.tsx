...
export const generatePDF = (
	...
	try {
		...
		if (!formData || !formData.prestations) {
			console.error(
				'Les données du formulaire sont manquantes ou incorrectes.'
			);
			return;
		}
		...
		const documentTitle =
			formData.documentType === 'quote' ? 'Devis' : 'Facture';
		doc.text(documentTitle, 15, 20);
		doc.setTextColor('#374151');
		doc.setFontSize(20);
		doc.setFont('DM Sans', 'bold');
		const documentNumberPrefix =
			formData.documentType === 'quote' ? 'N° D-' : 'N° F-';
		doc.text(`${documentNumberPrefix}${formData.quoteNumber}`, 15, 30);

		doc.setFont('DM Sans', 'bold');
		doc.setFontSize(14);
		doc.text(
			`${(formData.company || '[Entreprise]').toUpperCase()}`,
			15,
			50
		);
		...
		const siren_comp = process.env.COMPANY_SIREN
			? `N° SIREN: ${process.env.COMPANY_SIREN}\n`
			: '';
		const addressParts_comp = process.env.COMPANY_ADDRESS
			? process.env.COMPANY_ADDRESS.split(', ').map((part: string) =>
					part ? part + '\n' : ''
			  )
			: ['[ADRESSE]\n', '[CODE POSTAL]\n'];
		const address1_comp = addressParts_comp[0]
			? addressParts_comp[0].toUpperCase()
			: '';
		const address2_comp = addressParts_comp[1]
			? addressParts_comp[1].toUpperCase()
			: '';
		const email_comp = process.env.COMPANY_EMAIL
			? process.env.COMPANY_EMAIL + '\n'
			: '';
		const phone_comp = process.env.COMPANY_PHONE
			? process.env.COMPANY_PHONE + '\n'
			: '';
		doc.text(
			`${siren_comp}${address1_comp}${address2_comp}${email_comp}${phone_comp}`,
			115,
			55
		);
		...
		const validity = `${validityDate.getDate()}/${
		...
		autoTable(doc, {
			...
			body: (formData.prestations || []).map(
				(service: Service, index: number) => [
					...
					`${(
						parseFloat(service.price.toString()) * service.quantity
					).toFixed(2)}€`,
				]
			),
		...
		const totalHT = (formData.prestations || []).reduce(
			(acc: number, s: Service) =>
				acc + parseFloat(s.price.toString()) * s.quantity,
			0
		);
		...
		const addTotals = (doc: jsPDF, yPosition: number) => {
			...
			doc.text(`${totalTVA.toFixed(2)}€`, 195, yPosition + 15, {
				align: 'right',
			});
			doc.text(`TOTAL TTC :`, 150, yPosition + 20, { align: 'right' });
			doc.text(`${totalTTC.toFixed(2)}€`, 195, yPosition + 20, {
				align: 'right',
			});
		};
		...
		const addadditionalInfo = (doc: jsPDF, yPosition: number) => {
			...
			let additionalInfo = processPlaceholders(
